...
export class HealthCheckRoutes {
  ...
  private async pingController(req: Request, res: Response): Promise<void> {
    const result = HealthCheckService.ping();
    res.status(200).send(result);
  }

  private async pingDbController(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
